...
class stock_picking(models.Model):
    ...
    def get_picking_invoice_partner(self):
        if self.sale_id:
    ...
    def _prepare_invoice(self):
        ...
        inv_vals = {'partner_id': self.partner_id.id,
                    'move_type': context.get('invoice_type'), }
        if self.sale_id:
            inv_vals.update(self.sale_id._prepare_invoice())
        elif self.purchase_id:
            inv_vals.update(self.purchase_id._prepare_invoice())
        inv_vals.update({
...
class wizard_stock_picking_invoice(models.TransientModel):
    ...
    def default_get(self, fieldslist):
        ...
        usage = pick.move_ids[0].location_id.usage if type == 'incoming' else pick.move_ids[0].location_dest_id.usage
    ...
    def create_invoice(self):
        ...
        for picking_id in picking_ids.filtered(lambda l:l.state == 'done'):
            if picking_id.company_id.id != self.journal_id.company_id.id:
                raise ValidationError(_("Selected Journals company and Picking company are different."))
            if picking_id.invoice_ids or not picking_id.move_ids:
                continue
            if self.group_by_partner:
                key = picking_id.get_picking_invoice_partner()
#                 key = picking_id.partner_id.id
#                 if picking_id.sale_id:
#                     key = picking_id.sale_id.partner_invoice_id.id
            else:
                key = picking_id.id
