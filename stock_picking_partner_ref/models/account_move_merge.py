...
class AccountMove(models.Model):
    ...
    def _get_first_invoice_fields(self, invoice):
        return {
            ...
            "partner_ref": [invoice.partner_ref or ""],
    ...
    def do_merge(
        ...
        * Invoice lines are exactly the same except for the quantity and unit
        ...
        for account_invoice in self._get_draft_invoices():
            ...
            partner_refs = seen_partner_ref.setdefault(invoice_key, set())
            ...
            if not invoice_infos:
                ...
                if (
                    ...
                    and account_invoice.partner_ref not in partner_refs
                ):
                    partner_refs.add(account_invoice.partner_ref)
                    invoice_infos["partner_reference"] = (
                       ", ".join(invoice_infos.get("partner_ref"))
                       + ", "
                       + str(account_invoice.partner_ref)
                    )

                # if account_invoice.partner_ref and account_invoice.partner_ref not in partner_refs:
                #     partner_refs.add(account_invoice.partner_ref)
                #
                #     current_partner_reference = invoice_infos.get("partner_reference", "")
                #     if current_partner_reference:
                #         current_partner_reference += " "
                #
                #         invoice_infos["partner_reference"] = (
                #                 current_partner_reference + str(account_invoice.partner_ref)
                #         )



                # if account_invoice.partner_ref:
                #     current_partner_reference = invoice_infos.get("partner_reference", "")
                #     if current_partner_reference:
                #         current_partner_reference += " "
                #
